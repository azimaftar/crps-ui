...
@Component({
  selector: 'app-maklumat-profil-main',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './maklumat-profil-main.component.html',
  styleUrl: './maklumat-profil-main.component.scss'
})
export class MaklumatProfilMainComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (history.state && history.state.dariCapJariEmpatPost) {
      ...
      console.log("✅ Datang dari cap-jari-four-post-main");
    }
  }
  ...
  goNext(): void {
    ...
    if (index !== -1 && index + 1 < this.steps.length) {
      this.router.navigate(['../maklumat-visa-main'], { relativeTo: this.route });
    }
  }
