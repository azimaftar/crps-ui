...
@Component({
  ...
  templateUrl: './pilihan-dokumen.component.html',
  styleUrl: './pilihan-dokumen.component.scss'
})
export class PilihanDokumenComponent {
  // icons = flagSet;
  travelerRecords: any[] = [];
  ...
  onImbasDokumenClick(docType: string): void {
    ...
    if (docType === 'passport' || docType === 'kadABTC') {
      this.router.navigate(['/ibc/pemeriksaan-masuk-kbc/imbasan-passport'], {
        queryParams: { docType }
      });
    } else if (docType === 'myKad') {
      this.router.navigate(['/ibc/pemeriksaan-masuk-kbc/imbasan-mykad'], {
        queryParams: { docType }
      });
    } else {
      this.router.navigate(['/ibc/pemeriksaan-masuk-kbc/imbasan-passport'], {
