...
@Component({
  selector: 'app-memo-periksa-keluar-masuk',
  imports: [
    CommonModule,
    ReactiveFormsModule,
    FormsModule,
  ...
  templateUrl: './memo-periksa-keluar-masuk.component.html',
  styleUrls: ['../menu-pertanyaan.component.scss']
})


export class MemoPeriksaKeluarMasukComponent implements OnInit {
  ...
  pageTitle = 'Senarai Permohonan Memo Periksa Keluar Masuk';
  ...
  onSubmit(): void {
    ...
    this.subjectService.getGroupedSubjects().subscribe((groupedData) => {
      ...
      if (!this.nationality || !this.passportNumber || !this.noMemo) {
