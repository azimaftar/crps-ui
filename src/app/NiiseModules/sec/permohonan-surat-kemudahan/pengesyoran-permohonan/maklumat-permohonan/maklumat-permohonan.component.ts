...
@Component({
  selector: 'app-maklumat-permohonan',
  imports: [
  CommonModule,
      ReactiveFormsModule,
      RouterModule,
...
export class MaklumatPermohonanComponent implements OnInit {
  ...
  private loadSelectedData(): void {
    ...
    // 1. From router state (fresh navigation)
    // 2. From stored state (returning from next page)
    // 3. From URL parameters (if implemented)
    // 4. Default sample data (fallback)
