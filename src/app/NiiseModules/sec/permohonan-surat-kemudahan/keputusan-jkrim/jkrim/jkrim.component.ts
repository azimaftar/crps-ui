...
@Component({
  ...
  templateUrl: './jkrim.component.html',
  styleUrls: [
  '../../permohonan-surat-kemudahan.component.scss'
]

})
export class JkrimComponent implements OnInit {
  ...
  catatan: string = '';
  namaPelulus: string = '';
  idPengguna: string = '';  // Added missing field
  tarikhKelulusan: string = '';
  tarikhNapNas: string = '';  // Added missing field
  ...
  private initializeComponent(): void {
    console.log('Pengesyoran Permohonan 3 component initialized');
  }
  ...
  private redirectToFirstPage(): void {
    this.clearState();
    this.router.navigate(['/sec/permohonan-surat-kemudahan/keputusan-jkrim']);
  }
  ...
  private validateForm(): boolean {
    ...
    if (!this.selectedSyorOption) {
      errors.push('Radio button pilihan');
    }

    if (!this.namaPelulus.trim()) {
      errors.push('Nama Pengguna');
    }

    if (!this.idPengguna.trim()) {
      errors.push('ID Pengguna');
    }

    if (!this.tarikhKelulusan) {
      errors.push('Tarikh Keputusan Permohonan JKRIM');
    }

    if (!this.tarikhNapNas) {
      errors.push('Tarikh NAP/ NAS');
    }

    // Check catatan if "Tidak Lulus" is selected
    if (this.selectedSyorOption === 'tidak_syor' && !this.catatan.trim()) {
      errors.push('Catatan (wajib diisi apabila pilihan "Tidak Lulus" dipilih)');
    }
  ...
  // Added method for ID Pengguna
  onIdPenggunaChange(event: Event) {
    if (!this.isViewMode) {
      const target = event.target as HTMLInputElement;
      this.idPengguna = target.value;
    }
  }
  ...
  // Added method for Tarikh NAP/NAS
  onTarikhNapNasChange(event: Event) {
    if (!this.isViewMode) {
      const target = event.target as HTMLInputElement;
      this.tarikhNapNas = target.value;
    }
  }
}