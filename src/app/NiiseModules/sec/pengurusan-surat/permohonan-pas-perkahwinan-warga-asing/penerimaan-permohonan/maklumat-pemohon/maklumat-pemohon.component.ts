...
export class MaklumatPemohonComponents {
  ...
  CitizenshipCategory: string[] = [
    ...
    'Warga Asing',
  ]; // bound to the dropdown for citizenship status
    ...
    this.permohonanService.getApplInfo(icNumber).subscribe({
      next: (response) => {
        ...
        if (response && response.records && response.records.length > 0) {
          ...
          const genderObj = this.genderOptions.find(
            (g) => g.sexCD === genderCode
          );
          this.gender = genderObj ? String(genderObj.sexCD) : '';

          const CitizenCode = record.selectedCitizenCategory || '';
          const CitizenObj = this.countryOptions.find(
            (c) => c.wrldSTACode === CitizenCode
          );
  ...
  async saveSubmitForm(): Promise<void> {
