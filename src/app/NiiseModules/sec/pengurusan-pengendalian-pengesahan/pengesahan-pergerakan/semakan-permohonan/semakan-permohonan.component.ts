...
@Component({
  selector: 'app-semakan-permohonan-pengesahan-pergerakan',
...
export class SemakanPermohonanComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.result = this.permohonanFormDataService.getKeputusanData();
  ...
  steps = [
    ...
    { number: 2, title: 'Maklumat Permohonan', isAccessible: false, route: '/sec/pengurusan-pengendalian-pengesahan/pengesahan-pergerakan/semakan-permohonan/maklumat-permohonan' },
    { number: 3, title: 'Keputusan Permohonan', isAccessible: false, route: '/sec/pengurusan-pengendalian-pengesahan/pengesahan-pergerakan/semakan-permohonan/keputusan-permohonan' }
  ];

  navigateToStep(steps: number): void { 
    this.permohonanFormDataService.setKeputusanData(this.result);
  ...
  onSearch() {
    ...
    this.defaultKeputusanData = this.permohonanFormDataService.getDefaultKeputusanData();

    if (this.result) {
      this.showNoResults = false;
      this.mergedData = { ...this.defaultKeputusanData, ...this.result};
      this.permohonanFormDataService.setKeputusanData(this.mergedData);
    } else {
      this.showNoResults = true;
