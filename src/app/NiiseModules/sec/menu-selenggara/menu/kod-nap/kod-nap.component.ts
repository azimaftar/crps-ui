...
@Component({
  ...
  templateUrl: './kod-nap.component.html',
  styleUrls: ['../menu.component.scss']
})
export class KodNapComponent {
  ...
  // Validation
  errors = { kod: false, keteranganBM: false, keteranganEN: false, catatan: false };
  ...
  constructor(private router: Router, private kodService: KodService) {
    ...
    if (state?.selectedItem) {
      ...
      this.kod = item.napTypeCode || '';
      this.keteranganBM = item.napTypeBmDesc || '';
      this.keteranganEN = item.napTypeBiDesc || ''; 
    }
  }

  saveForm() {
  this.errors = { kod: false, keteranganBM: false, keteranganEN: false, catatan: false };
  let valid = true;

  if (!this.kod.trim()) { this.errors.kod = true; valid = false; }
  if (!this.keteranganBM.trim()) { this.errors.keteranganBM = true; valid = false; }
  if (!this.keteranganEN.trim()) { this.errors.keteranganEN = true; valid = false; }
  ...
  const payload: any = {
    ...
    code: this.kod,
    ctznshipCat: 'AG1',
    descBM: this.keteranganBM,
    descBI: this.keteranganEN,
    createId: 'zaidi',
  ...
  if (this.mode === 'tambah') {
    ...
    this.kodService.updateKodNap(payload).subscribe({
      next: (res) => this.handleResponse(res),
      error: (err) => {
...
goBack() { this.router.navigate(['/sec/menu-selenggara/menu']); }
  ...
  isStepActive(stepNumber: number): boolean { return this.currentStep === stepNumber; }
}
