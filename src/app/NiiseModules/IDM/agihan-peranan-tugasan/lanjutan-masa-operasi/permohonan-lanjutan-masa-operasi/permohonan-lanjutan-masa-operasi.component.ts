...
@Component({
  selector: 'permohonan-lanjutan-masa-operasi',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './permohonan-lanjutan-masa-operasi.component.html',
  styleUrls: ['./permohonan-lanjutan-masa-operasi.component.scss']
})
export class PermohonanLanjutanMasaOperasiComponent implements OnInit {
...
pegawaiToDeleteIndex: number | null = null;

removePegawai(index: number) {
  this.pegawaiToDeleteIndex = index;  // store the index to delete

  this.modalTitle = 'Adakah anda pasti ingin memadamkan rekod?';
  this.modalButtons = [
    { label: 'Tidak', action: 'close', class: 'btn-all' },
    { label: 'Ya', action: 'removePegawai', class: 'btn-submit' }
  ];
  ...
  tambahPegawai(): void {
    ...
    selectedPegawai.forEach(p => {
      ...
      this.modalTitle = 'Pegawai baru telah berjaya ditambah.';
      this.modalButtons = [
        { label: 'Tutup', action: 'closeTambahPegawai', class: 'btn-submit' }
      ];
...
handleModalPopupAction(event: any) {
  ...
  if (event.action === 'removePegawai' && this.pegawaiToDeleteIndex !== null) {
    this.newPegawaiList.splice(this.pegawaiToDeleteIndex, 1);
    this.pegawaiToDeleteIndex = null; // reset after deletion
    this.showModalPopup = false;
  } else if (event.action === 'closeTambahPegawai') {
  ...
  submitRequest(): void {
    ...
    if (this.newPegawaiList.length === 0) {
      alert('Sila tambah sekurang-kurangnya satu pegawai');
      return;
    }
