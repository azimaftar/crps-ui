...
export class MaklumatPenetapanPerananComponent implements OnInit {
  ...
  exportPeranan(): void {
    const dataStr = JSON.stringify(this.perananList, null, 2);
    const blob = new Blob([dataStr], { type: 'application/json' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `peranan-data-${new Date().toISOString().split('T')[0]}.json`;
    link.click();
  }
  ...
  goToPrevious(): void {
    ...
    this.router.navigate(['/IDM/pendaftaran-id-pengguna/sokongan-permohonan-id/maklumat-profil-pegawai']);
  }

  goToNext(): void {
    // Navigation logic to next step (not in template)
    console.log('Navigating to next step');
    this.router.navigate(['/IDM/pendaftaran-id-pengguna/sokongan-permohonan-id/maklumat-ptj-selepas-data-dihapus']);
  ...
  // Modal Methods
  openPenetapanPerananModal(): void {
