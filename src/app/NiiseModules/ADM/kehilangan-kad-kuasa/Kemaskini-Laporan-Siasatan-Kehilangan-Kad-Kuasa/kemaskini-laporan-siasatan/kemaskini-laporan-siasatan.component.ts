...
interface TableItem {
  ...
  noDokumen: string;
  pangkat: string;
  tarikhTerimaLaporan: string;
  statusSiasatan: string;
}

@Component({
  selector: 'app-kemaskini-laporan-siasatan',
  imports: [
    CommonModule,
    CardModule,
    GridModule,
  ...
  templateUrl: './kemaskini-laporan-siasatan.component.html',
  styleUrl: './kemaskini-laporan-siasatan.component.scss'
})
export class KemaskiniLaporanSiasatanComponent implements OnInit {
  icons = { cilSync };
  ...
  data: TableItem[] = [
    {
      ...
      statusSiasatan: 'Menunggu Pengesahan'
    },
    {
      checkbox: false,
  ...
  navigateToMaklumatLaporanSiasatan() {
    this.router.navigate([
      'adm/pemulangan-perampasan-kad-kuasa/kemas-kini-maklumat-arahan-pemulangan-perampasan-kad-kuasa/maklumat-laporan-siasatan',
    ]);
  }

  navigateToKemasKiniMaklumatArahan() {
    this.router.navigate([
      'adm/pemulangan-perampasan-kad-kuasa/kemas-kini-maklumat-arahan-pemulangan-perampasan-kad-kuasa/kemas-kini-maklumat-arahan',
    ]);
  }
  ...
  search() {
    ...
    // Filter by statusSiasatan
    if (searchValues.statusSiasatan && searchValues.statusSiasatan.trim()) {
      filtered = filtered.filter(item =>
        item.statusSiasatan.toLowerCase().includes(searchValues.statusSiasatan.toLowerCase())
      );
    }

    // Filter by tarikhTerimaLaporan (date range)
    if (searchValues.tarikhDari || searchValues.tarikhHingga) {
      filtered = filtered.filter(item => {
        const itemDate = new Date(this.convertDateToISO(item.tarikhTerimaLaporan));
