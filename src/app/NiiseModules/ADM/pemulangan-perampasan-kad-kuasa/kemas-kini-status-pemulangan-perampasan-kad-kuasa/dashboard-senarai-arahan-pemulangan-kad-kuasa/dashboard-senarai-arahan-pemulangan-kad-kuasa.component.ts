...
interface TableItem {
  ...
  noDokumen: string;
  pangkat: string;
  tarikhPengesahan: string;
  statusPengesahan: string;
  statusKemasKini: string;
}
...
export class DashboardSenaraiArahanPemulanganKadKuasaComponent implements OnInit {
  ...
  data: TableItem[] = [
    {
      ...
      tarikhPengesahan: '20/06/2025',
      statusPengesahan: 'Disahkan',
      statusKemasKini: 'Belum Kemas Kini'
    },
    {
      checkbox: false,
  ...
  navigateToKemasKiniStatusPemulanganPerampasanKadKuasa() {
    this.router.navigate([
      'adm/pemulangan-perampasan-kad-kuasa/kemas-kini-status-pemulangan-perampasan-kad-kuasa/kemas-kini-status-pemulangan-perampasan-kad-kuasa',
    ]);
  }
  ...
  search() {
    ...
    // Filter by statusKemasKini
    if (searchValues.statusKemasKini && searchValues.statusKemasKini.trim()) {
      filtered = filtered.filter(item =>
        item.statusKemasKini.toLowerCase().includes(searchValues.statusKemasKini.toLowerCase())
      );
    }
    ...
    if (searchValues.tarikhDari || searchValues.tarikhHingga) {
      filtered = filtered.filter(item => {
        const itemDate = new Date(this.convertDateToISO(item.tarikhPengesahan));
