...
export interface ReqDaftarPengagihanLokasiBertugas {
  division_cd: string;
  unit_cd: string;
  group: string;
  tarikh?: string; 
...
export interface RespDaftarPengagihanLokasiBertugas {
  Staff_ID: string;
  Kaunter: string;
  Name: string;
...
export class DaftarPengagihanLokasiBertugasService {
  private apiUrl = 'http://localhost:8080/BKP';

  constructor(private http: HttpClient) {}

  getMaklumatCarian(filters: ReqDaftarPengagihanLokasiBertugas): Observable<RespDaftarPengagihanLokasiBertugas[]> {
    ...
    return this.http.get<RespDaftarPengagihanLokasiBertugas[]>(`${this.apiUrl}/getMaklumatLokasi`, { params });
  }
}